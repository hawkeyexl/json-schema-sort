...
describe('Integration Tests', () => {
  it('should work end-to-end with complex nested structures', () => {
    ...
    expect(Object.keys(result.user)).to.deep.equal(['personalInfo', 'preferences']);

    // Check deeply nested personalInfo
    expect(Object.keys(result.user.personalInfo)).to.deep.equal(['name', 'age', 'email']);

    // Check preferences
    expect(Object.keys(result.user.preferences)).to.deep.equal(['theme', 'language']);

    // Check settings
    expect(Object.keys(result.settings)).to.deep.equal(['notifications', 'privacy']);

    // Check that additional properties (metadata) are sorted alphabetically at the end
    expect(Object.keys(result.metadata)).to.deep.equal(['createdAt', 'version']);
  });
